          ...
          <div className="flex flex-row justify-normal space-x-3 items-center w-fit">
            {user?.isLoggedIn === "true" ? (
              <div className="flex flex-row space-x-3 items-center">
                Hi
                <p className="text-orange-400 text-sm underline ml-1">
                  {user?.firstname}
                </p>
                <button
            ...
            <h4 className="hidden md:block text-sm no-underline hover:underline">
              <Link href={"/login"}>Daily Deals</Link>
            </h4>

            <h4 className="hidden md:block text-sm no-underline hover:underline">
              <Link href={"/login"}>Help & Contact</Link>
            </h4>
          </div>
          ...
          <div className="flex flex-row justify-center items-center space-x-3 min-w-max">
            ...
            <h4 className="text-sm no-underline hover:underline">
              ...
              <div className="hidden md:flex flex-row items-center justify-start">
                <HoverCard>
                  <HoverCardTrigger>Watchlist</HoverCardTrigger>
                  <HoverCardContent>
                    The React Framework – created and maintained by @vercel.
                  </HoverCardContent>
                </HoverCard>
                <ChevronDown
                  className={cn("h-6 w-6 transition-all text-muted-foreground")}
                />
              </div>
              {/* </Link> */}
            </h4>
            ...
            <h4 className="text-sm no-underline hover:underline">
              ...
              <div className="flex flex-row items-center justify-start">
                <HoverCard>
                  <HoverCardTrigger>My nKan&apos;s</HoverCardTrigger>
              ...
              <SheetTrigger asChild>
                <div className="relative flex items-center cursor-pointer">
                  <span className="absolute flex bottom-4 right-0 md:left-2 h-5 w-5 bg-black justify-center items-center rounded-full text-white">
                    <p className="self-center justify-center text-sm mb-2">
                      {items.length}
                    </p>
                  </span>
                  <ShoppingCart />
                  ...
                  <>
                    <div className="flex flex-col space-y-2 items-center w-full mb-5">
                      {items.map((item) => (
                        <div
                          key={item.id}
                          className="border-b border-gray-300 flex flex-row w-full items-center justify-between"
                        >
                          <div className="flex flex-row items-start space-x-5 py-3">
                            ...
                            <div className="flex flex-col items-start justify-center space-y-1 w-full">
                              <h2 className="line-clamp-1 leading-relaxed ">
                                {item.description}
                              </h2>
                              {item.count > 1 ? (
                      ...
                      <SheetClose asChild>
                        ...
                        >
                          <Button
                            variant="default"
                            type="submit"
                            className="w-full text-lg h-10 justify-center items-center"
                          >
                            Continue
                          </Button>
                        </Link>
